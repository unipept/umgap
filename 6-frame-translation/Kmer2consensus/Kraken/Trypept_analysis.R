...
identifications <- numeric()
  ...
  filename <- paste0("Kraken_MiSeq.11.pept.csv")
...
header <- c("kingdom","phylum","class","order","family","genus","species")

library(tikzDevice)
tikz("HiSeq_trypept_ids.tex",width=5,height=3.5)
plot(nr_dataframe$A_hydrophila,type='n',ylim=c(-200,1000),xaxt='n',main="HiSeq,tryptische peptiden",
...
axis(1, at=1:8, labels=c("ids",header),cex.axis=0.75)
...
tikz("MiSeq_trypept_ids.tex",width=5,height=3.5)
plot(nr_dataframe$V_cholerae,type='n',ylim=c(-200,1000),xaxt='n',main="MiSeq,tryptische peptiden",

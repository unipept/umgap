...
fn test_two_on_same_path() {
    ...
    let calculator = TreeBasedAggregator::new(taxon_list, false);
...
#[test]
fn test_two_on_fork() {
    let calculator = TreeBasedAggregator::new(fixtures::taxon_list(), false);
    assert_eq!(1, calculator.aggregate(&vec![2, 10239]).id);
    assert_eq!(1, calculator.aggregate(&vec![10239, 2]).id);

    let calculator = TreeBasedAggregator::new(fixtures::taxon_list(), true);
